...
```
    ...
    ```{r, message=FALSE, echo=FALSE}
sjPlot::tab_model(ols.chile, show.se = TRUE, string.se = "S.E.",
                  dv.labels = c("OLS"), string.pred = "Variables", 
                  string.ci = "C.I. (95%)", p.style = "stars", 
                  file = "output.html")
```
...
-   Theory & Hypothesis: I argue that with different levels of presidential power, losers' satisfaction with democracy should also be different. In countries with powerful presidents, losers should be more unsatisfied with how the government works, for being the political minority in the system causes them to be largely excluded from the policy-making process. In countries with weaker presidents, losers should be less unsatisfied, for their policy interests still can be possibly realized by the premier and the cabinet, which is determined by the congressional election.
    ...
    *Hypothesis: Losers will be more satisfied with democracy when the
    president is weaker.*
    ...
    The data can be assessed [here](https://drive.google.com/file/d/1kveKsgSGcAIKI86qrW71imC71Hvv60Oi/view?usp=sharing). All the variables inside are well coded and already remove NAs.
...
knitr::kable(df %>% count(country, eyear, prespow1) %>% dplyr::select(-n), digits = 4)
```
...
library(stargazer)
stargazer(logit.swd, logit.swd.int, 
          column.sep.width="1pt", font.size="small", digits=5,
          title = "The Effect of Electoral Outcomes and Presidential Power on Satisfaction with Democracy (Logistic Regression Model).",
          dep.var.labels = c("Satisfaction with Democracy"),
          covariate.labels = c("Loser", "Presidential Power", "Divided Government",
                               "Male", "Education Level", "Age", 
                               "Loser * Presidential Power"),
          column.labels = c("Additive Model", "Interactive Model"), 
          star.cutoffs = c(0.05, 0.01, 0.001), type = "text")
```

What does the coefficient of `Loser` mean? The regression outputs for
additive and interactive models here are in such form:

Additive Model:

$$
\begin{aligned}
log \frac{\pi_i}{1-\pi_i} & =\alpha + \beta_{Loser} Loser_i + \beta_{Prespow} Prespow_i + \beta_{Male} Male_i + \beta_{Age} Age_i + \beta_{Education} Education_i + \beta_{DividedGov} DividedGov_i \\
&= 0.80402-0.83762Loser_i-1.11516Prespow_i+0.00797Male_i+0.00954Age+0.06027Education_i-0.05990DividedGov_i
\end{aligned}
$$
Interactive Model:
...
$$
\begin{aligned}
log \frac{\pi_i}{1-\pi_i} &= \alpha + \beta_{Loser} Loser_i + \beta_{Prespow} Prespow_i + \beta_{Loser*Prespow} (Loser*Prespow)_i + \beta_{Male} Male_i + \beta_{Age} Age_i + \beta_{Education} Education_i + \beta_{DividedGov} DividedGov_i \\
&=0.68884-0.69264Loser_i-0.65411Prespow_i-0.60460(Loser*Prespow)_i + 0.00855Male_i+0.00987Age+0.06064Education_i-0.05880DividedGov_i
\end{aligned}
$$
where $\pi_i$=Pr(Being satisfied with democracy)
...
2.  Akaike Information Criterion (AIC)
    ...
    </center>$AIC=-2(lnL_M-P)=Dev_M+2P$ where $P$ is the number of parameters in the model (including $\alpha$)</center>
...
- The ordinal representation of the variable uses many
categories (e.g., 8 or more) and cases are distributed somewhat evenly
...
Again, we can transform the log-odds coefficients to odds ratios by
exponentiating them.
...
If you want to get odds ratios with confidence intervals, you can use
`confint()` to find them.
...
Note that the summation of the predicted probabilities for each category
of the outcome variable should be 1!
